...
if (isset($_POST['login'])) {
	if (isset($_POST['email']) && isset($_POST['password'])) {
		...
		$result =$mysqli->query("SELECT * from user WHERE (email='$user_login') AND password='$password_login_md5' AND activation='yes'");
		$num = $result->num_rows;
		...
		if ($num>0) {
			...
			$result1 =$mysqli->query("SELECT * from user WHERE (email='$user_login') AND password='$password_login_md5' AND activation='no'");
...
if(isset($_POST['activate'])){
	if(isset($_POST['actcode'])){
		...
		$result2 =$mysqli->query("SELECT * from user WHERE (email='$user_login') AND confirmCode='$user_acccode'");
		...
		if ($num3>0) {
			...
			$_SESSION['user_login'] = $get_user_uname_db;
			setcookie('user_login', $user_login, time() + (365 * 24 * 60 * 60), "/");
			$mysqli->query("UPDATE user SET confirmCode='0', activation='yes' WHERE email='$user_login'");
			if (isset($_REQUEST['ono'])) {
				$ono = $mysqli->real_escape_string($_REQUEST['ono']);
				header("location: orderform.php?poid=".$ono."");
			}else {
